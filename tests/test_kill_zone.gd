...
func test_kill_zone_kills_and_respawns_a_bot() -> void:
	...
	spawn_marker.add_to_group(Team.spawn_group_name(Team.A)) # a bot's default team_id
...
func test_kill_zone_respawns_player_at_configured_marker() -> void:
	...
	spawn_marker.add_to_group(Team.spawn_group_name(Team.A)) # the player's default team_id
